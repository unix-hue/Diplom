...
public class TaskCreationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] people = new String[]{"Василий Пупкин,vasya@gmail.com", ""};
        if(!DeskActivity.getUser().equals(people[0]))
            people[1] = DeskActivity.getUser();
        ...
        button.setOnClickListener(v -> {
            if (name_edit.getText().length() != 0 && desc.getText().length() != 0) {
                DeskActivity.addListItem(getIntent().getIntExtra("index", 0), new Task(name_edit.getText().toString(), desc.getText().toString(), worker));
                this.finish();
            }else{
